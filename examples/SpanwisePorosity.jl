...
begin
	...
	function nondimensionalize(d,f::Quantity{T,WavenumberDim,S}) where {T,S}
		return NoUnits(x*reference_length(d))
	end
	function nondimensionalize(d,f::Quantity{T,FrequencyDim,S}) where {T,S}
...
# ╔═╡ 55f7e082-1b58-4be8-a5e3-ed7cd77833ed
begin
...
begin
	...
	numerical_options = Dict(
		:space=>Legendre(SqrtLineS{-1/4}(0.0,1)),
							# approximation space
